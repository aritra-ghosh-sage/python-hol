...
import api
from api import ConfigResponse, ConfigUpdateRequest
...
async def get_config() -> ConfigResponse:
    ...
    return ConfigResponse(
        semantic_top_k=api._config.semantic_top_k,
        keyword_top_k=api._config.keyword_top_k,
        final_top_k=api._config.final_top_k,
        semantic_weight=api._config.semantic_weight,
        keyword_weight=api._config.keyword_weight,
        enable_rerank=api._config.enable_rerank,
        pre_rerank_top_k=api._config.pre_rerank_top_k,
    ...
    try:
        ...
        if collection_changed:
            ...
            if api._config.collection_name in existing:
                ...
                logger.info("Switched to existing collection '%s'", api._config.collection_name)
            else:
            ...
            logger.info("Retriever re-initialized with collection '%s'", api._config.collection_name)
        ...
        if api._cache is not None:
            try:
                ...
                logger.warning("Failed to clear cache after config update: %s", e)
        else:
        ...
        try:
            ...
            logger.error("Failed to persist configuration to disk: %s", e)
            raise HTTPException(
                status_code=500,
        ...
        logger.warning("Invalid configuration parameter: %s", e)
        raise HTTPException(
            status_code=400,
